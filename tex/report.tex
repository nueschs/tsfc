...
\section{Conclusion}
The presented work shows how fuzzy logic can be applied to social data accessed from Twitter. More so, the implementation described in chapter~\ref{implementation} demonstrates that this is possible in a simple, strait-forward matter. Due to the limited time frame in which this work was conducted, clearly there are many open issues and unexplored possibilities remaining (cf. chapter~\ref{future_work}). However, the report at hand undoubtedly shows the convenience of the applied solution as well as its potential.
